...
4. **Portfolio analytics** — *complete (2026-06-15)*. Ported `portfolioAnalytics` (adapted to tradebot's per-position realized PnL model) + MetricStrip: win rate, realized PnL, fee drag, avg hold, open exposure, skip rate, fees paid, copied count; plus a PnL-by-token table. `GET /analytics`; pure `derivePortfolioAnalytics` (unit-tested) behind a `getPortfolioAnalytics` query (integration-tested). Manual copies count automatically (they create normal fills/positions). Closed trade = a position with `closedAt`; win/loss judged on its realized PnL.
5. **Reprocess tool** — *complete (2026-06-15)*. Ported `candidateReprocess` as `pnpm reprocess [path…]` (defaults to `./recordings`): re-runs the current decoder over recorded events and diffs the signals it *would* produce against the persisted `trade_signals` for those txs — reporting status upgrades, side flips, copy-token gains, and newly/no-longer-derived. Pure `summarizeReprocess` (unit-tested) in `packages/decoder`; CLI in `scripts/reprocess.ts` mirroring `verify:ledger`. Read-only over trading state (never writes signals/fills/positions; the shared decoder may additively cache token metadata for a never-seen token).
6. **Links/names** — *complete (2026-06-15)*. Token symbol/name hydration + contract→explorer links shipped 2026-06-12; this finishes it with `WalletLink` (address → explorer + GMGN.ai) on the leaders and settings pages and `TxLink` (txHash → explorer) on the feed and candidates pages. URL helpers (`explorerAddressUrl`, `explorerTxUrl`, `gmgnWalletUrl`) in `lib/api`. Feed/candidates leader identity stays a UUID (no on-chain address there), so only tx is linked.
...
8. **Uniswap V4 pricing** — *Parts 1–4 + P6 complete (2026-06-16)*. Design in `docs/uniswap-v4-pricing-plan.md` and `docs/adr/0002-price-uniswap-v4-via-decoded-poolid.md`. Root cause of LOCAL trades skipping with `no-liquidity-data`: pricing scanned only Uni V3 + Aerodrome CL, never the V4 singleton PoolManager (no on-chain pool discovery by pair). Fix prices a V4 pool by the **poolId observed in the leader's swap**. P1: decoder surfaces the V4 Swap `id` (poolId); `extractV4PoolId` captures it for any single-V4-swap tx so balance-delta signals carry it too. P2: `poolId` on `TradeSignal` + `trade_signals.pool_id` (migrations 0006/0007). P3: pricing reads V4 via **StateView** (`getSlot0`/`getLiquidity` by poolId), currencies from the swap pair, liquidity = virtual quote reserve (`method: "v4-virtual-reserves"`); optional `MarketHint` threaded through `getUsdPrice(Result)`/`getLiquidityUsd(Result)`; fixed a wrong Base V4 PoolManager constant in `venues.ts`. P4: engine passes `v4MarketHint(signal)` into buy-decision and `handleConfirmed`. Live-check tool `pnpm check-v4 <txHash>`. **Marks + exit-sell depth follow-up complete (2026-06-17):** `getV4MarketHintForToken` recovers `{poolId, counterCurrency}` from prior signals (indexed via migration 0008) so the marks job and `executeExitSell` re-price V4-only positions instead of skipping/penalising them. **TTL-only pricing caches now bounded (resolved):** `cappedSet` + `MAX_CACHE_ENTRIES=5000` in `packages/pricing/src/price.ts` cap `llamaCache`/`marketCache`/`chainlinkCache` (evict oldest first); tested in `price.test.ts`. **Still deferred (with reason):** full decoder→engine pipeline not yet re-checked on a fresh live LOCAL trade — needs a real trade post-deploy, pre-fix rows have null poolId.

### Phase 10 — Polymarket paper copy-trading (agreed 2026-06-21)

Promote Polymarket from **record-only** (the `PolymarketWatcher` writes every trade as a `candidate`, decoupled from the engine) to a **first-class paper copy-trade source** — scored eventually, auto-copyable, filled, marked, and exited. Agreed with the user in a grill session; the seven design decisions, forced consequences, and rejected alternatives are recorded in **`docs/adr/0003-polymarket-copy-trading-as-a-parallel-path.md`** (which **amends ADR 0001**). Terminology — **Outcome Share**, **Resolution**, and the widened **Trade Signal** — lives in `CONTEXT.md`. Build the parts **in order**; each ends with `pnpm build && pnpm test` green and a commit.

**Core principle (ADR 0003):** Polymarket is a **parallel path**, not a third chain through the EVM pipeline. The engine throws on non-EVM by design (`PaperEngine.evm()`), and the AMM assumptions (pool discovery, reserves, quote conversion, TWAP) have no CLOB equivalent. Reuse only the data model (`TradeSignal`, `positions`, FIFO `accounting`) and the decision knobs (sizing, the auto-copy toggle); everything pricing/veto-shaped is Polymarket-specific. Three exit modes, of which **resolution** ($1/$0 forced settlement) is new and has no EVM analog. **Money-accounting parts (3, 5) are Opus-authored and go through `/code-review` before `main`; the CLOB/Gamma clients and jobs (1, 2, 6) are GLM-draftable.**

**Vetoes for the Polymarket buy path** differ from the EVM five: **reuse** weight, auto-copy-off, staleness, blocklist, insufficient-balance; **drop** the AMM liquidity-USD/tier-mute, TWAP-divergence, and fallback-price vetoes; **add** a `max-spread` risk filter and a hard `market-closed`/`market-resolved` correctness gate (copying a buy into a resolved market is the prediction-market rug).

1. **Persist `conditionId` + outcome mapping** — *complete (2026-06-21)*. Added nullable `trade_signals.condition_id` + `outcome_index` (migration 0012), extended `TradeSignal`, and carried both fields through `tradeToCandidateSignal` plus the store insert/upsert/rowToSignal round-trip so Polymarket rows retain the market/outcome identity needed for later resolution settlement.
2. **CLOB price source** — *complete (2026-06-21)*. Added `packages/pricing/src/polymarket.ts`: public CLOB `/price` wrapper with zod boundary validation, injectable `fetchImpl`, bounded per-token cache, bid/ask inference from the two side reads, and midpoint-based `spreadBps`. Added config/env `POLYMARKET_CLOB_API_URL` and `POLYMARKET_MAX_SPREAD_BPS`, exported via `packages/pricing/src/index.ts`, and covered malformed/non-200 responses plus cache reuse/eviction in unit tests.
3. **Engine entry path** — a Polymarket-aware `handlePolymarketSignal` (and copy-sell path) that **bypasses `evm()`**: prices via the CLOB source at copy-time (best ask for buys, bid for sells), applies the Polymarket veto set above, reuses `sizeBuy`/`applyTradeToState`/`applySellToState`. Positions reuse the existing table keyed by the CTF tokenId as `tokenAddress` (already how the watcher stores it). Sizing reuses `equity × BASE_TRADE_PCT × weight` (weight is constant 1.0 in v1, Part 5-deferred). *(Opus-authored; `/code-review` before merge.)*
4. **Auto-copy trigger** — a parallel polling job (mirrors `startCandidateCopyJob`) that reads new confirmed polygon signals for auto-copy-enabled leaders and routes them to `handlePolymarketSignal`. The watcher stays decoupled (writes rows, no bus emit); the EVM bus/decoder/hot path is untouched. With **pure parity** (Decision 7), promote confidently-decoded Polymarket trades to normal signals (`decodeStatus` decoded, not `candidate`) so auto-copy-off produces an `auto-copy-off` skip fill, not a review-queue entry. *(GLM-draftable; the routing into the Opus-authored engine method is the boundary.)*
5. **Marks + resolution settlement** — *complete (2026-06-22)*. Added `packages/pricing/src/polymarketMarks.ts` plus runner wiring so open Polygon outcome-share positions are re-marked from CLOB midpoint prices and pushed through `engine.ingestPriceMark(...)`, bringing them into equity/portfolio valuation without touching the EVM marks job. Extended `packages/pricing/src/polymarket.ts` Gamma `/markets` parsing to retain `outcomes`, `outcomePrices`, and `clobTokenIds`, exported `getPolymarketResolutionPayout(...)`, added `getOpenPolymarketPositionsForSettlement(...)` in the store, and implemented `PaperEngine.settlePolymarketPosition(...)` so resolved positions force-close at $1/$0 through the shared `applySellToState` accounting path. The runner now starts separate Polygon marks + resolution jobs, and package tests plus `pnpm build` are green.
6. **UI + leaders** — *complete (2026-06-22)*. Polymarket positions/fills already render through the shared feed/portfolio/analytics views. Added `isPolymarketOutcome` + `polymarketProfileUrl` helpers in `apps/web/src/lib/api.ts`: `tokenTitle` renders outcome shares as "Yes — market question", `TokenLink` suppresses the opaque CTF tokenId (no explorer link exists for it), and `WalletLink` now adds a Polymarket profile link for polygon leaders alongside the polygonscan link. Adding a Polymarket leader = add a polygon wallet with auto-copy (already supported by `getActiveWallets(db,"polygon")`). Covered by `apps/web/src/lib/api.test.ts`; `pnpm build && pnpm test` green.

**Deferred (with reason):** **Scoring** — Polymarket leaders run at baseline weight 1.0 in v1; mixing them into the EVM z-score cohort is meaningless (bounded 0–1 returns vs. unbounded memecoin variance) and realized round-trips accrue slowly (resolution-gated). Separate-cohort scoring is the eventual target (ADR 0003 §Decision 6). **Cherry-pick review** — the per-trade manual-approval path is EVM-only for now; revisit if hand-picking prediction-market bets proves valuable (would use `reviewStatus`, not `candidate` status, to avoid re-overloading the term).

**Open tuning (not design):** `POLYMARKET_MAX_SPREAD_BPS`, staleness threshold for poll-lagged signals, marks/resolution poll cadence, whether `MIN_NOTIONAL_USD` needs a floor for sub-dollar share prices, and whether `proportional` sizing is meaningful for Polymarket or should force `fixed`.
