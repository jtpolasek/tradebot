...
- Hybrid GMGN plan (2026-06-12): 6-part feature plan agreed — full plan in memory file `hybrid-gmgn-plan.md`, terminology in `CONTEXT.md`, key decision in `docs/adr/0001-persist-candidates-outside-scoring.md`. **Part 1 (candidate persistence) in progress**, in order: (a) cap backfill window (~30 min of blocks; larger gaps log + skip to live), (b) engine staleness veto via block timestamp (skip reason `stale-signal`) — both fix restarts copying hours-old trades at current price; then (c) add `decode_status`/`confidence`/`reason` to `trade_signals`, stop dropping ambiguous decodes, engine acts on `decoded` only, scorer counts `decoded` only.
