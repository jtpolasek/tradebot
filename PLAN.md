...
5. **Marks + resolution settlement** — two jobs, both polygon-only and off the EVM marks/exit jobs: (a) a **marks writer** that re-prices open polygon positions via the CLOB source so equity/valuation (and later scoring) work; (b) a **resolution settler** that polls Gamma by `condition_id`, and on settlement force-closes any still-open copied position at $1 (winning outcome) / $0 (losing) through `applySellToState`, realizing PnL even if the leader never sold. *(Settlement accounting is Opus-authored; the Gamma client is GLM-draftable.)*
6. **UI + leaders** — surface Polymarket positions/fills in the existing feed/portfolio/analytics views (they're normal fills/positions, so most works for free); ensure `WalletLink` resolves polygon addresses to a Polymarket profile and outcome tokens render as "Yes/No — market question" (the watcher already upserts those labels). Adding a Polymarket leader = add a polygon wallet with auto-copy (already supported by `getActiveWallets(db,"polygon")`). *(GLM-draftable.)*
