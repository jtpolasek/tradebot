...
1. **Persist `conditionId` + outcome mapping** — the watcher currently drops `conditionId`; resolution settlement can't map a held CTF tokenId → win/lose payout without it. Add `condition_id` (+ outcome index) to `trade_signals` (nullable, EVM leaves null) and carry it through `tradeToCandidateSignal`; migration + `rowToSignal` round-trip. *(GLM-draftable.)*
2. **CLOB price source** — new Polymarket price module (`packages/pricing`, parallel to `findBestMarket`, never indexes `rpcClients`): `getPolymarketPrice(tokenId, side)` → CLOB `GET /price` (best ask/bid) and a spread read from `/price` both sides (or `/book` top). Boundary-validated with zod like `client.ts`; injectable `fetchImpl`; bounded cache. Config: `POLYMARKET_CLOB_API_URL`, `POLYMARKET_MAX_SPREAD_BPS`. *(GLM-draftable.)*
